...
export const config: TemplateConfig = {
  stream: {
    ...
    filter: {
      ...
      savedFilterIds: [
        "dm_tropical-smoothie_address_countrycode"
      ]
    },
      ...
      if (entity.dm_baseEntityCount == 1) {
        entity.dm_directoryChildren.map((res: any) => {
          ...
          res.dm_directoryChildren ? res.dm_directoryChildren.map((detl: any) => {
            ...
            if (!detl.slug) {
              let slugString = detl.name.toLowerCase();
              let slug =slugString;
              detlslug1 = `${detl.id}-${constant.slugify(slug)}`;
            } else {
          ...
          <div className="container">
            <div className="sec-title">
              <h2 style={{ textAlign: "center" }}>
                {StaticData.AllRegion} {document.c_addressCountryDisplayName}{" "}
              </h2>
            </div>
